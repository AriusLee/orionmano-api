...
def build_readme_sheet(ws):
    """README sheet — engagement metadata, methodology weights, sheet index,
    manual-input checklist, refresh notes, disclaimer. Modeled on the TP
    workpaper's `Dashboard_UserGide` cover."""
    write_header_band(ws, 1, "Valuation Workpaper — Cover & User Guide", span=6)
    ws.cell(row=2, column=1, value=(
        "Auto-generated by the Orionmano valuation pipeline. Read this sheet first. "
        "Every figure on every downstream tab is driven by a named range on the Inputs sheet."
    )).font = SMALL_FONT
    ws.merge_cells(start_row=2, start_column=1, end_row=2, end_column=6)
    ...
    def kv(row: int, label: str, formula: str, fmt: str | None = None) -> None:
    ...
    # ── Engagement metadata ────────────────────────────────────────────
    section(4, "Engagement metadata")
    kv(5, "Company", "=company_name")
    ...
    kv(18, "Reporting currency", "=currency")
    kv(19, "Unit", "=unit")
    kv(20, "Secondary currency", "=currency_alt")
    kv(21, "FX rate to alt", "=fx_rate_alt", fmt="0.0000")
    ws.cell(row=22, column=2, value="Note").font = SMALL_FONT
    ws.cell(row=22, column=3, value=(
        'All monetary values throughout this workbook are denominated in '
        '[currency] [unit] unless explicitly stated otherwise.'
    )).font = SMALL_FONT
    ws.merge_cells(start_row=22, start_column=3, end_row=22, end_column=6)

    # ── Methodologies ──────────────────────────────────────────────────
    section(24, "Valuation methodologies & weights")
    headers = ["Methodology", "Weight", "Sheet", "Notes"]
    for i, h in enumerate(headers, start=2):
        ...
        c.border = BORDER
    rows = [
        ("Discounted Cash Flow (DCF)",   "=weight_dcf",       "DCF / DCF (Independent)", "Two-scenario WACC; Gordon-growth or exit-multiple terminal."),
        ("Comparable Companies (CoCo)",  "=weight_comps",     "Comps / CoCo Multiples",  "Q1/Median/Q3 NTM trading multiples × target metrics."),
        ("Precedent Transactions",       "=weight_precedent", "Precedent",              "Mean EV/EBITDA × Y1 EBITDA from observed M&A deals."),
        ("Net Asset Value (NAV)",        "=weight_nav",       "—",                      "Asset-based; typically 0% for going-concern targets."),
    ]
    for i, (m, w, sheet_ref, notes) in enumerate(rows):
        r = 26 + i
        ...
        ws.cell(row=r, column=4, value=sheet_ref).font = NORMAL_FONT
        n = ws.cell(row=r, column=5, value=notes)
        n.font = SMALL_FONT
        n.alignment = Alignment(wrap_text=True)
        ws.merge_cells(start_row=r, start_column=5, end_row=r, end_column=6)
    # Sum check
    total_row = 30
    ws.cell(row=total_row, column=2, value="Total weight").font = Font(bold=True, size=10, italic=True)
    s = ws.cell(row=total_row, column=3, value="=weight_dcf+weight_comps+weight_precedent+weight_nav")
    s.number_format = "0.0%"
    s.font = Font(bold=True, size=10)
    ws.cell(row=total_row, column=4, value='Must equal 100%').font = SMALL_FONT
    ws.merge_cells(start_row=total_row, start_column=4, end_row=total_row, end_column=6)

    # ── Sheet index ────────────────────────────────────────────────────
    section(33, "Sheet index")
    ...
    for i, h in enumerate(headers, start=2):
        ...
        c.alignment = Alignment(horizontal="center")
        c.border = BORDER
    sheet_index = [s for s in SHEETS if s[0] != "_dropdowns"]
    for i, (name, desc) in enumerate(sheet_index):
        r = 35 + i
        ws.cell(row=r, column=2, value=str(i + 1)).font = NORMAL_FONT
        ws.cell(row=r, column=3, value=name).font = Font(bold=True, size=10)
        d = ws.cell(row=r, column=4, value=desc)
        d.font = SMALL_FONT
        d.alignment = Alignment(wrap_text=True)
        ws.merge_cells(start_row=r, start_column=4, end_row=r, end_column=6)

    next_row = 35 + len(sheet_index) + 2

    # ── Manual-input checklist ─────────────────────────────────────────
    section(next_row, "Items requiring post-export manual input")
    next_row += 1
    headers = ["#", "Item", "Sheet", "Cell / range", "Notes"]
    for i, h in enumerate(headers, start=2):
    ...
    next_row += 1
    manual_items = [
        ("1", "Selected football-field band (analyst override)",
         "Inputs / Football Field",
         "selected_low / selected_mid / selected_high",
         "Producer auto-fills initial estimate (±15% around weighted mid). Adjust to your judgment band before finalizing."),
        ("2", "shares_outstanding_diluted",
         "Inputs",
         "shares_outstanding_diluted",
         "Required for diluted per-share output. Pull from prospectus / cap table if not auto-filled."),
        ("3", "Coco/Precedent metric values flagged as null",
         "CoCo Multiples / Margins / Ratios / Precedent",
         "Cells with INPUT_FILL background",
         "Producer leaves a metric null when source data is unavailable. Fill from Capital IQ / Bloomberg if you obtain it."),
        ("4", "Engagement team names",
         "Inputs",
         "engagement_team_partner / engagement_team_manager",
         "Defaults to 'TBD' if engagement letter not provided. Replace with actual partner / manager names."),
    ]
    for it in manual_items:
        for i, val in enumerate(it, start=2):
    ...
    # ── Refresh notes ──────────────────────────────────────────────────
    section(next_row, "Reference data — refresh schedule")
    next_row += 1
    refresh_items = [
        ("Country ERP",      "Annual",  "Damodaran NYU Stern country risk premium",       "pages.stern.nyu.edu/~adamodar/"),
        ("Industry Averages", "Annual",  "Damodaran US industry betas / margins / multiples", "pages.stern.nyu.edu/~adamodar/"),
        ("Risk-free rate",   "Per engagement", "10y UST yield as at valuation date",      "Bloomberg / Capital IQ"),
        ("ERP",              "Per engagement", "Implied / historical ERP for mature markets", "Damodaran implied ERP"),
    ]
    headers = ["Dataset", "Cadence", "Source", "URL / vendor"]
    for i, h in enumerate(headers, start=2):
        c = ws.cell(row=next_row, column=i, value=h)
        c.font = HEADER_FONT
        c.fill = HEADER_FILL
    ...
    # ── Disclaimer ─────────────────────────────────────────────────────
    section(next_row, "Disclaimer & use restriction")
    next_row += 1
    disclaimer = (
        "This workpaper has been prepared for the express purpose stated above and for the "
        "exclusive use of the engaging party. It must not be reproduced, distributed, or "
        "relied upon by any other party without prior written consent. The valuation reflects "
        "conditions and information available as at the valuation date; subsequent events are "
        "not incorporated. All inputs derive from sources cited in the audit-trail columns of "
        "the Inputs sheet — refer to those entries for source documents and assumptions. "
        "This is not a fairness opinion, audit, or attestation engagement under PCAOB / "
        "AICPA standards unless explicitly designated."
    )
    c = ws.cell(row=next_row, column=2, value=disclaimer)
    c.font = SMALL_FONT
    c.alignment = Alignment(wrap_text=True, vertical="top")
    ws.merge_cells(start_row=next_row, start_column=2, end_row=next_row + 4, end_column=6)
...
def build_dashboard_sheet(ws):
    """Dashboard — primary client-facing summary. Modeled on the TP workpaper's
    Dashboard tab. Headline KPIs + methodology range + selected band + sensitivity
    + CoCo benchmark + backtesting. All values pull from named ranges and
    ...
    def kpi(row: int, col: int, label: str, formula: str, fmt: str = "#,##0") -> None:
        # Two-row KPI cell: small label on top row, large bold value below
        l = ws.cell(row=row, column=col, value=label)
        l.font = SMALL_FONT
    ...
    section(4, "Headline values (Per-Management scenario)")
    kpi(5, 3, "Enterprise Value",     "='Valuation Summary'!C7",  fmt="#,##0")
    kpi(5, 4, "Equity (after DLOM/DLOC)", "='Valuation Summary'!C10", fmt="#,##0")
    kpi(5, 5, "Per share (basic)",    "='Valuation Summary'!C16", fmt="#,##0.0000")
    kpi(5, 6, "WACC",                 "=wacc_per_mgmt",            fmt="0.00%")
    kpi(5, 7, "Terminal growth",      "=terminal_growth_rate",     fmt="0.00%")
    ws.row_dimensions[5].height = 16
    ws.row_dimensions[6].height = 26

    # ── Section 2: Independent scenario row ─────────────────────────────
    ws.cell(row=8, column=2, value="Independent scenario").font = Font(bold=True, italic=True, size=10)
    ws.cell(row=8, column=3, value="='Valuation Summary'!D7").number_format = "#,##0"
    ws.cell(row=8, column=4, value="='Valuation Summary'!D10").number_format = "#,##0"
    ws.cell(row=8, column=5, value="='Valuation Summary'!D16").number_format = "#,##0.0000"
    ws.cell(row=8, column=6, value="=wacc_indep").number_format = "0.00%"
    ws.cell(row=8, column=7, value="=terminal_growth_rate").number_format = "0.00%"
    for c in range(3, 8):
        ws.cell(row=8, column=c).font = Font(size=10)
        ws.cell(row=8, column=c).alignment = Alignment(horizontal="center")
        ws.cell(row=8, column=c).border = BORDER

    # ── Section 3: Methodology range ────────────────────────────────────
    section(11, "Methodology range — enterprise value")
    headers = ["Methodology", "Low", "Mid", "High", "Weight", "Weighted mid", "Notes"]
    for i, h in enumerate(headers, start=2):
    ...
    wa.fill = SECTION_FILL
    wa.border = BORDER
    wa.alignment = Alignment(horizontal="center")

    # Selected band row
    r += 1
    ws.cell(row=r, column=2, value="Selected band (low / mid / high)").font = Font(bold=True, size=10, italic=True)
    for j, ref in enumerate(["selected_low", "selected_mid", "selected_high"], start=3):
        c = ws.cell(row=r, column=j, value=f"={ref}")
        c.number_format = "#,##0"
        c.font = Font(bold=True, size=10)
        c.fill = EXTENDED_FILL
        c.border = BORDER
        c.alignment = Alignment(horizontal="center")

    # ── Section 4: CoCo benchmark check ─────────────────────────────────
    section(20, "CoCo trading multiples — benchmark stats")
    headers = ["Multiple", "Min", "Q1", "Median", "Q3", "Max", "Mean"]
    for i, h in enumerate(headers, start=2):
        c = ws.cell(row=21, column=i, value=h)
        c.font = HEADER_FONT
        c.fill = HEADER_FILL
        c.alignment = Alignment(horizontal="center")
        c.border = BORDER
    # Col positions in CoCo Multiples sheet — must match build_coco_metric_sheet
    # cols: 5=EV/Sales LTM, 6=EV/Sales NTM, 7=EV/EBITDA LTM, 8=EV/EBITDA NTM, 9=P/E LTM, 10=P/E NTM
    # Stat rows: 37=Max, 38=Q3, 39=Median, 40=Mean, 41=Q1, 42=Min  (must match _COCO_*_ROW)
    coco_rows = [
        ("EV / Sales NTM",   "F"),
        ("EV / EBITDA NTM",  "H"),
        ("P / E NTM",        "J"),
    ...
    for i, (label, col_letter) in enumerate(coco_rows):
        r = 22 + i
        ws.cell(row=r, column=2, value=label).font = NORMAL_FONT
        # Min, Q1, Median, Q3, Max, Mean (note _stat_rows order in build_coco_metric_sheet)
        for col_idx, stat_row in enumerate([42, 41, 39, 38, 37, 40]):
            c = ws.cell(row=r, column=3 + col_idx,
                        value=f"='CoCo Multiples'!{col_letter}{stat_row}")
            c.number_format = "0.0×;-0.0×;\"n/a\""
            c.font = NORMAL_FONT
            c.border = BORDER
            c.alignment = Alignment(horizontal="center")
            if col_idx == 2:  # Median bold
                c.font = Font(bold=True, size=10)

    # ── Section 5: Sensitivity grid (preview) ───────────────────────────
    section(30, "Sensitivity — EV (WACC × terminal growth, Per-Mgmt)")
    ws.cell(row=31, column=2, value=(
        "Full grid is on the Sensitivity tab. This panel shows EV at the base case "
        "(centre cell) plus ±1 step in each direction."
    )).font = SMALL_FONT
    ws.merge_cells(start_row=31, start_column=2, end_row=31, end_column=8)

    headers = ["WACC ↓ / g →", "g − 1 step", "Base g", "g + 1 step"]
    for i, h in enumerate(headers, start=2):
        ...
        c.alignment = Alignment(horizontal="center")
        c.border = BORDER
    # Sensitivity grid in `Sensitivity` sheet: rows 7-13, cols B(WACC)/C..I(g axis)
    # Base cell is at the centre. With wacc_count=5, base_row is row 7+5=12;
    # but the layout writes 2*count+1 rows starting at row 7. We'll reference
    # rows around the middle. To be robust against count, use the named-range-driven
    # base and ±1 lookup via INDEX:
    sens_lookups = [
        ("WACC − 1 step", "INDEX('Sensitivity'!C7:M17,sens_wacc_count,sens_terminal_g_count)",     "INDEX('Sensitivity'!C7:M17,sens_wacc_count,sens_terminal_g_count+1)",     "INDEX('Sensitivity'!C7:M17,sens_wacc_count,sens_terminal_g_count+2)"),
        ("Base WACC",      "INDEX('Sensitivity'!C7:M17,sens_wacc_count+1,sens_terminal_g_count)",   "INDEX('Sensitivity'!C7:M17,sens_wacc_count+1,sens_terminal_g_count+1)",   "INDEX('Sensitivity'!C7:M17,sens_wacc_count+1,sens_terminal_g_count+2)"),
        ("WACC + 1 step", "INDEX('Sensitivity'!C7:M17,sens_wacc_count+2,sens_terminal_g_count)",   "INDEX('Sensitivity'!C7:M17,sens_wacc_count+2,sens_terminal_g_count+1)",   "INDEX('Sensitivity'!C7:M17,sens_wacc_count+2,sens_terminal_g_count+2)"),
    ]
    for i, (label, *cells) in enumerate(sens_lookups):
        r = 33 + i
        lc = ws.cell(row=r, column=2, value=label)
        lc.font = NORMAL_FONT
        lc.border = BORDER
        for j, formula in enumerate(cells):
            c = ws.cell(row=r, column=3 + j, value=f"={formula}")
            c.number_format = "#,##0"
            c.font = NORMAL_FONT
            c.alignment = Alignment(horizontal="center")
            c.border = BORDER
    ...
    section(38, "Backtesting — historical revenue vs Y0 base")
    ws.cell(row=39, column=2, value=(
        "Sanity check: Y0 base revenue (cascading into Projections) should equal the most-recent "
        "FY-1 audited revenue."
    )).font = SMALL_FONT
    ws.merge_cells(start_row=39, start_column=2, end_row=39, end_column=8)
    headers = ["", "FY-3", "FY-2", "FY-1", "Y0 (Projections)", "Δ Y0 vs FY-1"]
    for i, h in enumerate(headers, start=2):
        c = ws.cell(row=40, column=i, value=h)
        c.font = HEADER_FONT
        c.fill = HEADER_FILL
        c.alignment = Alignment(horizontal="center")
        c.border = BORDER
    bt_rows = [
        ("Revenue",  "='Historical FS'!E7",  "='Historical FS'!F7",  "='Historical FS'!G7",  "=revenue_y0", "=IFERROR(revenue_y0/'Historical FS'!G7-1,\"n/a\")"),
        ("EBITDA",   "='Historical FS'!E13", "='Historical FS'!F13", "='Historical FS'!G13", "—",           "—"),
    ]
    ...
    ws.cell(row=footer_row, column=2, value=(
        "Open the web dashboard (Orionmano · Companies · this entity · Valuation tab) for "
        "interactive Football Field, Sensitivity heatmap, FCFF projection, and CoCo multiple charts."
    )).font = SMALL_FONT
    ws.merge_cells(start_row=footer_row, start_column=2, end_row=footer_row, end_column=8)
...
def build():
    ...
    formula_builders = {
        ...
        "Dashboard": build_dashboard_sheet,
