...
- `projections` — years (typically 5), revenue_growth_method, **revenue_y0** (last reported full-year revenue, in the same currency × unit as the workpaper), **nwc_y0** (audited Y0 NWC = current assets ex-cash − current liabilities ex-debt), and Y1-Y5 arrays for revenue_growth, **revenue_growth_primary**, gross_margin, opex_pct_revenue, capex_pct_revenue, dep_pct_revenue, nwc_pct_sales (all 7 arrays, each length-5). **revenue_y0 is the cascade base — without it the workpaper math is dead.** ALSO populate **gross_profit_y0**, **opex_y0** (negative), **ebitda_y0**, **ebit_y0**, **tax_y0** (negative), **net_income_y0** from the most recent full-year audited income statement (`historical_fs.gross_profit[-1]`, `historical_fs.opex_total[-1]`, `historical_fs.ebitda[-1]`, `historical_fs.ebit[-1]`, `historical_fs.tax_expense[-1]`, `historical_fs.net_income[-1]`) so the Projections sheet's Y0 column shows real audited values, not blanks. Eric 2026-05-18 / 2026-05-19 #1. **These six Y0 absolutes MUST be expressed in the same `currency.unit` as `revenue_y0` — if the workpaper unit is `'000` and revenue_y0 is `1584`, then gross_profit_y0 is `858`, NOT `858000`.** Emitting them in raw currency units while revenue_y0 is scaled makes the Y0 gross margin render as tens of thousands of percent. Sanity-check before emitting: `gross_profit_y0 / revenue_y0` must land between 0 and 1. `opex_y0` and `tax_y0` must be NEGATIVE (the Projections cascade adds them: EBITDA = gross profit + opex).
...
- `projections.segments` (OPTIONAL but RECOMMENDED when the target has distinct business lines; REQUIRED when user-defined additional revenue streams are supplied in the context) — array of segments, each `{{name, start_year, revenue_y0 (if start_year=0) or initial_revenue (if start_year>0), revenue_growth[], gross_margin[] OR cogs_pct[], source ("core" | "additional_stream"), opex_pct_revenue[] (per-stream related S&M/distribution opex as % of stream revenue — REQUIRED for additional_stream segments so incremental costs scale with the stream), growth_basis (one-line defence of the growth vector, citing the market statistic and source used), contractual_support (contracts/backlog/MOUs supporting the stream; empty string if none)}}`. **Hard cap: at most 8 segments** (the workpaper's SEGMENT_ROWS). Each slot becomes an itemised line in the Projections revenue and COGS blocks; a 9th stream is rejected as an export error rather than silently dropped. If the target genuinely has more streams, consolidate the smallest into one named segment instead of emitting extras. When provided, total revenue & gross profit at each year are the SUM across all active segments; the top-level revenue_growth + gross_margin arrays are then IGNORED for the cascade (still emit them consistent with the segment aggregate — the pipeline re-derives them deterministically). Segments with `start_year > 0` model new revenue streams launched mid-projection (Eric 2026-05-08 item 2: "user can add new revenue streams with corresponding COGS during the projection period"). The sum of all segment revenue_y0 (for start_year=0 segments) MUST equal the top-level revenue_y0 — otherwise the workpaper Y0 base mismatches the audited FS.
